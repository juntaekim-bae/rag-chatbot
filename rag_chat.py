...
_MCQ_RULES = """【객관식 문제 출력 형식 — 반드시 준수】
문제에 선지(①②③④⑤ 또는 1~5번)가 있는 객관식 문제일 경우, 아래 순서와 형식을 정확히 따르라.

▶ STEP 1 — 빈칸·공란 분석 (문제에 (가)(나)(다) 등 빈칸이 있는 경우에만)
각 빈칸에 들어갈 내용을 문서에서 찾아 설명하라.
예)
(가): ~에 해당하는 내용
(나): ~에 해당하는 내용

▶ STEP 2 — 선지별 분석
각 선지가 맞는지 틀린지 근거와 함께 간략히 설명하라.
예)
① ~ : 문서에 따르면 ~ 이므로 [적절 / 부적절]
② ~ : ...
...
---
정답: X번
(모범답안 파일이 컨텍스트에 있으면 "모범답안 기준"을 괄호에 명시)
이유: 선지 분석을 종합한 최종 근거를 2~4문장으로 서술하라.
---

【모범답안 참조 규칙】
컨텍스트에 모범답안 파일이 포함된 경우 해당 파일에서 정답 번호를 최우선으로 찾아라.
모범답안 표기 형식 예: '1번-③', '2. ①', '3번: ②' 등.
모범답안이 없는 경우 문서 내용을 근거로 판단하여 번호를 제시하라."""
